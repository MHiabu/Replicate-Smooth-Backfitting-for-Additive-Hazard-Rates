...
get.alpha.new<-function(O1,time,status,alpha,alpha.1,dx,K.X.b,K.b,dX.b,dX0.b,Y,k,x.grid,n.grid,d,n,l)
{
    ...
    if (k==1) taylor.alpha.minusk <-  apply(as.matrix(taylor.alpha$taylor.alpha.i[,-1]),1,prod) else  
        {taylor.alpha.minusk <- apply(as.matrix(taylor.alpha$taylor.alpha.i[,-k]),1,prod) * matrix(taylor.alpha$taylor.alpha.i.0, ncol=n.grid[[1]], nrow=n, byrow = TRUE)
    }
...
#     alpha_backfit[[1]]<-alpha_backfit[[1]]*mean(alpha_backfit[[k]])
#     alpha.1_backfit[[1]]<-alpha.1_backfit[[1]]*mean(alpha_backfit[[k]])
#     alpha_backfit[[k]]<-alpha_backfit[[k]]/ mean(alpha_backfit[[k]])
#     alpha.1_backfit[[k]]<-alpha.1_backfit[[k]]/ mean(alpha_backfit[[k]])}
#   
...
# }
return(list(alpha_backfit=alpha_backfit,alpha.1_backfit=alpha.1_backfit,l=l,x.grid=x.grid,dx=dx))
}
